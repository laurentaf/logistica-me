-- This script runs all tests and populates the test_results table
-- Run with: dbt run --model test_results

-- First ensure test_results table exists
{{ config(
    materialized='table',
    tags=['test_reporting']
) }}
...
WITH test_executions AS (
  ...
  FROM {{ ref('endpoint_referential_integrity') }}
  ...
  SELECT 
    ...
    COUNT(*),
    'Missing hourly data detection'
  FROM {{ ref('missing_timestamp_ranges') }}
  
  UNION ALL
  
  SELECT 
    'business_logic',
    'anomalous_response_times',
    COUNT(*),
    'Statistical anomaly detection'
  FROM {{ ref('anomalous_response_times') }}
)
